namespace Mapbox.Examples
{
	...
	public class QuadTreeCameraMovement : MonoBehaviour
	{
	 ...
	 void Start()
		...
		public void Update()
		{
			_spawnedObject.transform.localPosition = _mapManager.GeoToWorldPosition(_location, true);

			_spawnedObject.transform.localScale = new Vector3(10,10,10);
	 ...
	 private void UpdateGPSData()
{
    if (Input.location.status == LocationServiceStatus.Running)
    {
        ...
        {
            Debug.Log("Latitude: " + currentLocation.latitude.ToString() + ", Longitude: " + currentLocation.longitude.ToString());
            UpdateMap(currentLocation.latitude, currentLocation.longitude);

            // Update the previous location to the current one for the next comparison
            previousLocation = currentLocation;
        }
    }

	  void UpdateMap(float lat,float lon){
		var position = new Vector2d(lat,lon);
		_mapManager.UpdateMap(position, 13.0f);

		_location = position;

		_markerPrefab.transform.localPosition = _mapManager.GeoToWorldPosition(position,true);
		_markerPrefab.transform.localScale = new Vector3 (10,10,10);
		_spawnedObject = _markerPrefab;
	}
